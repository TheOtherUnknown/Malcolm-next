...
class Utils(commands.Cog):
    ...
    @commands.command(description="Kicks a mentioned user from the server",
                      usage="@someone")
    ...
    async def nuke(self, ctx, num):
        if num is not None:
            num = int(num) # Num can't be >100. That needs to be checked at some point
            async for message in ctx.channel.history(limit=num):
                message.delete()
            await asyncio.sleep(1.2) # Sleep for a bit so everything gets deleted on time
