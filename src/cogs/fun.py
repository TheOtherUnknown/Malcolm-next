...
class Fun(commands.Cog):
    ...
    async def bofh(self, inter: nextcord.Interaction):
        ...
        lines = open('data/excuses.txt').read().splitlines(
    ...
    async def roll(self,
                   ...
                   sides: int = nextcord.SlashOption(
                       default=6, description="Number of sides on the dice"),
                   rolls: int = nextcord.SlashOption(
                       default=1, description="Times to roll the dice")):
        """Rolls a dice"""
        if rolls > 0 and rolls <= 32:
            if sides > 0 and sides <= sys.maxsize:
                embed = nextcord.Embed(title="Dice Rolls")
                for i in range(rolls):
                    embed.add_field(name=f"Roll {i+1}",
                                    value=random.randint(1, sides))
                await inter.send(embed=embed)
            else:
                await inter.send(
                    f"Sides cannot be less than 1 or exceed {sys.maxsize}")
        else:
